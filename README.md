...
- useInfiniteQuery를 활용해 검색 결과를 페이지 단위로 관리했습니다. queryKey에 검색어와 검색 옵션이 변경될 때 자동으로 refetch되도록 구현 했습니다. 호출은 10개씩 호출하도록 설정하고 getNextPageParam에서는 API의 is_end 값을 활용해 더 이상 데이터가 없으면 추가 요청을 중단하여 불필요한 네트워크 호출을 방지했습니다. 또한 enabled 옵션으로 검색어가 있을 때만 요청을 수행해 불필요한 API 호출을 막았습니다.
...
- useWindowVirtualizer를 사용해 리스트 가상화를 적용했습니다. 전체 데이터를 모두 렌더링하지 않고 현재 화면에 보이는 요소과 주변 요소(overscan)만 렌더링하여 DOM 노드 수를 줄였습니다. 이를 통해 스크롤 성능을 개선하고, 이미지가 많은 리스트에서도 메인 스레드의 렌더링 부하를 최소화했습니다.
